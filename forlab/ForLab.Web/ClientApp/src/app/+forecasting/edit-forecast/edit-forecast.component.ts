...
export class EditForecastComponent extends BaseService implements OnInit {
  ...
  loadingForecastInfo: boolean = false;
  ...
  loadDataById(id: number) {
    ...
    this.httpService.GET(ForecastInfosController.GetForecastInfoDetailsForUpdate, params)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(res => {
        if (res.isPassed) {
  ...
  setFormValue() {
    // Form
    this.form.controls["forecastInfoLevelId"].patchValue(this.forecastInfoDto.forecastInfoLevelId);
    this.form.controls["countryId"].patchValue(this.forecastInfoDto.countryId, { emitEvent: false });
    this.form.controls["scopeOfTheForecastId"].patchValue(this.forecastInfoDto.scopeOfTheForecastId);
    this.form.controls["name"].patchValue(this.forecastInfoDto.name);
    this.form.controls["startDate"].patchValue(this.forecastInfoDto.startDate, { emitEvent: false });
    this.form.controls["endDate"].patchValue(this.forecastInfoDto.endDate);
    this.form.controls["duration"].patchValue(this.forecastInfoDto.duration, { emitEvent: false });
    this.wastageRate = this.forecastInfoDto.wastageRate;
    // Methodology Form
    this.methodologyForm.controls["forecastMethodologyId"].patchValue(this.forecastInfoDto.forecastMethodologyId);
    this.methodologyForm.controls["isWorldHealthOrganization"].patchValue(this.forecastInfoDto.isWorldHealthOrganization);
    // Months
    if (this.forecastInfoDto.forecastMethodologyId == this.forecastMethodologyEnum.DempgraphicMorbidity) {
      let originalMonths = this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos;
      this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos = this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos?.filter((item, index) => {
        return this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos.findIndex(x => x.testId == item.testId && x.testingProtocolId == item.testingProtocolId && x.patientGroupId == item.patientGroupId) >= index;
      });
      this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos?.forEach(item => {
        item.testingProtocolCalculationPeriodMonthDtos = originalMonths.filter(x => x.testId == item.testId && x.testingProtocolId == item.testingProtocolId && x.patientGroupId == item.patientGroupId);
      });
      console.log(this.forecastInfoDto.forecastMorbidityTestingProtocolMonthDtos);
    }
    this._ref.detectChanges();
  }
  ...
  submitForm() {
    ...
    this.httpService.PUT(ForecastInfosController.UpdateForecastInfo, this.forecastInfoDto)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe(
        (res) => {
          if (res.isPassed) {
            this.loading = false;
            this.alertService.success('Forecast is updated successfully');
