...
import { InstrumentDto, InstrumentFilterDto } from 'src/@core/models/product/Instrument';
...
export class InstrumentsComponent extends BaseService implements OnInit {
  ...
  filterDto: InstrumentFilterDto = new InstrumentFilterDto();
  ...
  columns: TableColumn<InstrumentDto>[] = [
    ...
    { label: 'Max Through Put', property: 'maxThroughPut', type: 'int', visible: true },
    { label: 'Through Put Unit', property: 'throughPutUnitName', type: 'text', visible: true },
    { label: 'Testing Area', property: 'testingAreaName', type: 'text', visible: true },
    { label: 'Reagent System', property: 'reagentSystemName', type: 'text', visible: true },
    { label: 'Control Requirement Unit', property: 'controlRequirementUnitName', type: 'text', visible: true },
  ...
  loadData() {
    ...
    this.dataSource.loadObjects(InstrumentsController.GetAll, params);
  }
  ...
  createObject() {
    this.dialog.open(AddEditInstrumentComponent)
      .afterClosed()
  ...
  updateObject(instrument: InstrumentDto) {
    this.dialog.open(AddEditInstrumentComponent, {
      data: instrument
    })
      .afterClosed()
  ...
  updateIsActive(instrument: InstrumentDto) {
    this.dialog.open(ConfirmActiveComponent, {
      data: {
        url: InstrumentsController.UpdateIsActive,
        objectInfo: [{ key: 'Name', value: instrument.name }, { key: 'Vendor', value: instrument.vendorName }, { key: 'Testing Area', value: instrument.testingAreaName }],
        isActive: !instrument.isActive,
        queryParamsDto: { key: 'instrumentId', value: instrument.id },
      }
    })
      .afterClosed()
      .pipe(takeUntil(this.ngUnsubscribe))
  ...
  deleteObject(instrument: InstrumentDto) {
    this.dialog.open(ConfirmDeleteComponent, {
      data: {
        url: InstrumentsController.RemoveInstrument,
        objectInfo: [{ key: 'Name', value: instrument.name }, { key: 'Vendor', value: instrument.vendorName }, { key: 'Testing Area', value: instrument.testingAreaName }],
        queryParamsDto: { key: 'instrumentId', value: instrument.id },
      }
    })
  ...
  export() {
    ...
    this.dialog.open(ConfirmExportComponent, {
      data: {
        url: InstrumentsController.ExportInstruments,
  ...
  changeIsActiveForSelected(isActive: boolean) {
    ...
    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: InstrumentsController.UpdateIsActiveForSelected,
  ...
  changeSharedForSelected(shared: boolean) {
    ...
    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: InstrumentsController.UpdateSharedForSelected,
        ids: this.selection.selected.filter(x => x.createdBy == this.loggedInUser?.id || this.isSuperAdmin).map(x => x.id),
        shared: shared,
        type: 'shared'
      }
