...
@Component({
  selector: 'import-testing-areas',
  templateUrl: './import-testing-areas.component.html',
  styleUrls: ['./import-testing-areas.component.scss'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ImportTestingAreasComponent extends BaseService implements OnInit {

  // Table
  @Input('data') data: TestingAreaDto[] = [];
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  columns: TableColumn<TestingAreaDto>[] = [
    { label: 'Actions', property: 'actions', type: 'button', visible: true },
    { label: '#', property: 'index', type: 'custom', visible: true },
    { label: 'Name', property: 'name', type: 'text', visible: true },
  ];
  dataSource = new TableVirtualScrollDataSource();
  selection = new SelectionModel<TestingAreaDto>(true, []);
  // Form
  formArry: FormGroup[];
  // vars
  ...
  submit() {
    ...
    this.loading = true;
    let objectDtos: TestingAreaDto[] = [];

    objectDtos = this.formArry.map(element => {
      let val = new TestingAreaDto();
      val.name = String(element.value.name);
      return val;
    });

    this.httpService.POST(TestingAreasController.ImportTestingAreas, objectDtos)
