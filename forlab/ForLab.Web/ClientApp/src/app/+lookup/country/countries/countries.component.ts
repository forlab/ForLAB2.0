...
import { CountryDto, CountryFilterDto } from 'src/@core/models/lookup/Country';
...
import { CountryPeriodEnum, ContinentEnum } from 'src/@core/models/enum/Enums';
...
export class CountriesComponent extends BaseService implements OnInit {

  filterDto: CountryFilterDto = new CountryFilterDto();
  // Drp
  ...
  columns: TableColumn<CountryDto>[] = [
    ...
    { label: 'Flag', property: 'flag', type: 'image', visible: true },
    { label: 'Continent', property: 'continentName', type: 'text', visible: true },
    { label: 'Period', property: 'countryPeriodName', type: 'text', visible: true },
  ...
  loadData() {
    ...
    this.dataSource.loadObjects(CountriesController.GetAll, params);
  }
  ...
  updateIsActive(country: CountryDto) {
    this.dialog.open(ConfirmActiveComponent, {
      data: {
        url: CountriesController.UpdateIsActive,
        objectInfo: [{ key: 'Name', value: country.name }, { key: 'Native Name', value: country.nativeName }, { key: 'Short Code', value: country.shortCode }],
        isActive: !country.isActive,
        queryParamsDto: { key: 'countryId', value: country.id },
      }
    })
      .afterClosed()
  ...
  deleteObject(country: CountryDto) {
    this.dialog.open(ConfirmDeleteComponent, {
      data: {
        url: CountriesController.RemoveCountry,
        objectInfo: [{ key: 'Name', value: country.name }, { key: 'Native Name', value: country.nativeName }, { key: 'Short Code', value: country.shortCode }],
        queryParamsDto: { key: 'countryId', value: country.id },
      }
    })
  ...
  export() {
    ...
    this.dialog.open(ConfirmExportComponent, {
      data: {
        url: CountriesController.ExportCountries,
        fileName: 'Countries',
  ...
  changeIsActiveForSelected(isActive: boolean) {
    console.log(this.selection.selected.map(x => x.id));

    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: CountriesController.UpdateIsActiveForSelected,
