...
import { PatientGroupDto, PatientGroupFilterDto } from 'src/@core/models/lookup/PatientGroup';
...
import { AddEditPatientGroupComponent } from '../add-edit-patient-group/add-edit-patient-group.component';
...
@Component({
  ...
  styleUrls: ['./patient-groups.component.sass']
})
export class PatientGroupsComponent extends BaseService implements OnInit {

  filterDto: PatientGroupFilterDto = new PatientGroupFilterDto();

  // Table
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: false }) sort: MatSort;
  @ViewChild('searchInput', { static: false }) searchInput: ElementRef;
  columns: TableColumn<PatientGroupDto>[] = [
  ...
  selection = new SelectionModel<PatientGroupDto>(true, []);
  ...
  loadData() {
    ...
    this.dataSource.loadObjects(PatientGroupsController.GetAll, params);
  }
  ...
  createObject() {
    this.dialog.open(AddEditPatientGroupComponent)
      .afterClosed()
  ...
  updateObject(patientGroup: PatientGroupDto) {
    this.dialog.open(AddEditPatientGroupComponent, {
      data: patientGroup
    })
      .afterClosed()
  ...
  updateIsActive(patientGroup: PatientGroupDto) {
    this.dialog.open(ConfirmActiveComponent, {
      data: {
        url: PatientGroupsController.UpdateIsActive,
        objectInfo: [{ key: 'Name', value: patientGroup.name }],
        isActive: !patientGroup.isActive,
        queryParamsDto: { key: 'patientGroupId', value: patientGroup.id },
      }
    })
      .afterClosed()
      .pipe(takeUntil(this.ngUnsubscribe))
  ...
  deleteObject(patientGroup: PatientGroupDto) {
    this.dialog.open(ConfirmDeleteComponent, {
      data: {
        url: PatientGroupsController.RemovePatientGroup,
        objectInfo: [{ key: 'Name', value: patientGroup.name }],
        queryParamsDto: { key: 'patientGroupId', value: patientGroup.id },
      }
    })
  ...
  export() {
    ...
    this.dialog.open(ConfirmExportComponent, {
      data: {
        url: PatientGroupsController.ExportPatientGroups,
        fileName: 'PatientGroups',
  ...
  changeIsActiveForSelected(isActive: boolean) {
    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: PatientGroupsController.UpdateIsActiveForSelected,
  ...
  changeSharedForSelected(shared: boolean) {
    ...
    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: PatientGroupsController.UpdateSharedForSelected,
        ids: this.selection.selected.filter(x => x.createdBy == this.loggedInUser?.id || this.isSuperAdmin).map(x => x.id),
        shared: shared,
        type: 'shared'
      }
