...
import { RegionDto, RegionFilterDto } from 'src/@core/models/lookup/Region';
...
export class RegionsComponent extends BaseService implements OnInit {

  filterDto: RegionFilterDto = new RegionFilterDto();
  // Drp
  ...
  columns: TableColumn<RegionDto>[] = [
    ...
    { label: 'Country', property: 'countryFlag', type: 'image', visible: true },
    { label: 'Country Name', property: 'countryName', type: 'text', visible: true },
  ...
  loadData() {
    ...
    this.dataSource.loadObjects(RegionsController.GetAll, params);
  }
  ...
  createObject() {
    this.dialog.open(AddEditRegionComponent)
      .afterClosed()
  ...
  updateIsActive(region: RegionDto) {
    this.dialog.open(ConfirmActiveComponent, {
      data: {
        url: RegionsController.UpdateIsActive,
        objectInfo: [{ key: 'Name', value: region.name }],
        isActive: !region.isActive,
        queryParamsDto: { key: 'regionId', value: region.id },
      }
    })
      .afterClosed()
      .pipe(takeUntil(this.ngUnsubscribe))
  ...
  deleteObject(region: RegionDto) {
    this.dialog.open(ConfirmDeleteComponent, {
      data: {
        url: RegionsController.RemoveRegion,
        objectInfo: [{ key: 'Name', value: region.name }],
        queryParamsDto: { key: 'regionId', value: region.id },
      }
    })
  ...
  export() {
    ...
    this.dialog.open(ConfirmExportComponent, {
      data: {
        url: RegionsController.ExportRegions,
        fileName: 'Regions',
  ...
  changeIsActiveForSelected(isActive: boolean) {
    ...
    this.dialog.open(ConfirmActiveSelectedComponent, {
      data: {
        url: RegionsController.UpdateIsActiveForSelected,
