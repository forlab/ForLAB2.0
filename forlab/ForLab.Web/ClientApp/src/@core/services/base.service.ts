...
// Services
import { FileExtIcon, KeyValueObject, FormValidationError } from '../models/common/SharedModel';
...
export class BaseService implements OnDestroy {
  ...
  fileExtIcons: FileExtIcon[] = [
    { iconPath: './assets/img/files/doc.svg', supportedExt: ['doc', 'txt', 'docx'] },
    ...
    { iconPath: './assets/img/files/jpg.svg', supportedExt: ['png', 'jpg', 'jpeg', 'gif'] },
  ];
  //#endregion
  ...
  constructor(public injector: Injector) {
    ...
    this.loggedInUser = JSON.parse(localStorage.getItem('lab_user'));
    this.isSuperAdmin = this.permissionsService.getPermission('SuperAdmin') ? true : false;
  }
  ...
  formatInt(v, displayDecimal: boolean = true) {
    if (v) {
      return displayDecimal ? numeral(v).format('0,0.0000') : numeral(v).format('0,0');
    } else {
      return 0;
    }
  }
  ...
  formatPrice(p, currency: string = '$'): string {
    if (p) {
      ...
      return numeral(p).format(`0,0.0000`) + ' ' + currency.toUpperCase();
    } else {
      return '0';
    }
  }
  ...
  formatAddress(address) {
    if (address) {
      return `${address.street ? address.street : '-'}  ${address.suburb ? address.suburb : '-'}  ${address.state ? address.state : '-'}`;
    } else {
      return '-';
    }
  }
  ...
  getIconForFileType(ext: string): string {
    ...
    this.fileExtIcons.forEach(r => {
      if (r.supportedExt.includes(ext))
        result = r.iconPath;
    });
  ...
  formatAttachmentName(attachemntName: string, charsCount: number = 25): string {
    if (!attachemntName) {
      return '-';
    }
    ...
    if (name.length > charsCount) {
      ...
      result += attachemntName.substr(attachemntName.lastIndexOf('.')).toLowerCase();
      return result;
    }
  ...
  getFormValidationErrors(formArray: FormArray): FormValidationError[] {
      ...
      Object.keys(c1).forEach((c2, i2) => {
        Object.keys(c1['controls']).forEach((c3, i3) => {
          if (c1['controls'][c3].errors) {
            errors.push({
              message: `${c3} ${this.formatFormValidationMessage(c1['controls'][c3].errors)}`,
              rowNumber: i1 + 1
            });
          }
        });
    ...
    errors = errors.filter((item, index) => {
      return errors.findIndex(x => x.rowNumber == item.rowNumber && x.message == item.message) >= index;
    });

    return errors;
  }
  getFormGroupsErrors(formArray: FormGroup[], original: FormGroup[]): FormValidationError[] {
    let errors: FormValidationError[] = [];
