...
@Component({  
    selector: 'sa-ServiceList',  
    templateUrl: './ServiceStatisticList.component.html'
})  
export class ServiceListComponent implements AfterViewInit, OnInit  {  
 ...
 //dtOptions: Datatable.Settings = {};
  ...
  rowindex:number;
    ...
    delete(TestID) {  
            ...
            this._APIwithActionService.deleteData(TestID,'Consumption','Deleteservicestatistic').subscribe((data) => { 
    ...
    ngAfterViewInit(): void {
      document.querySelector('body').addEventListener('click', (event)=> {
        ...
        if (target.parentElement.className=='Edit') {
          this._router.navigate(["/ServiceStatistic/ServiceAdd",target.parentElement.getAttribute('data-demo-id')]);
          
        }
        if (target.parentElement.className=='del') {
          this.id =target.parentElement.getAttribute('data-demo-id');    
      ...
      smartModEg1() {
        this.notificationService.smartMessageBox({
          title: "Deletion",
          content: "Do you want to delete "+ this.id +" Service Staistic",
          buttons: '[No][Yes]'
        }, (ButtonPressed) => {
    ...
    ngOnInit() {
      ...
      this.DataView.options=
      {
         ...
         ajax: (data, callback, settings) => {
          this._APIwithActionService.getDatabyID('SERVICE_STATISTIC','MMProgram','GetForecastInfoByMethodology',"CID="+localStorage.getItem("countryid"))
          ...
          columns: [
              ...
              { data: 'lastUpdated' },
              {
                  render: (data, type, fullRow, meta) => {
                      return `
                      <a  class='Edit' data-demo-id="${fullRow.forecastID}"> <i class='fa fa-pencil-square-o'></i></a>
                      <a  class='del' data-demo-id="${fullRow.forecastID}"> <i class='fa fa-trash-o'></i></a>
                        `;
                  }
              }
          ] , 
          buttons: [
            {
              ...
              action: function ( e, dt, node, config ) {
               console.log()
               router.navigate(["/ServiceStatistic/ServiceAdd"])
