...
@Component({
    selector: 'app-forecast-new-prog-assum-testing',
    templateUrl: './ForecastNewProgAssumTesting.component.html',
    styleUrls: ['ForecastNewProgAssumTesting.component.css']
})

export class ForecastNewProgAssumTestingComponent implements OnInit {
    ...
    getgeneralassumption() {
        this._APIwithActionService.getDataList('MMProgram', 'GetGeneralAssumptionList').subscribe((resp) => {
            ...
            this.testingassumption = this.generalassumptionlist.filter(x =>
                x.programId === this.programId && x.assumptionType === 3 && x.isActive === "Yes");
        })
    }
    deleteAssumption(item) {
        ...
        this.bsDeleteModalRef.content.event.subscribe(res => {
            if (res.type == "delete") {
                var index = this.testingassumption.findIndex(x => x.id === item.id);
