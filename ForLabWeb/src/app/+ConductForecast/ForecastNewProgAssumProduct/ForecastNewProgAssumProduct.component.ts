...
@Component({
    selector: 'app-forecast-new-prog-assum-product',
    templateUrl: './ForecastNewProgAssumProduct.component.html',
    styleUrls: ['ForecastNewProgAssumProduct.component.css']
})

export class ForecastNewProgAssumProductComponent implements OnInit {
    ...
    getgeneralassumption() {
        this._APIwithActionService.getDataList('MMProgram', 'GetGeneralAssumptionList').subscribe((resp) => {
            ...
            this.productassumption = this.generalassumptionlist.filter(x =>
                x.programId === this.programId && x.assumptionType === 2 && x.isActive === "Yes");
            console.log('this.patientassumption', this.productassumption);
        })
    }

    deleteAssumption(item) {
        ...
        this.bsDeleteModalRef.content.event.subscribe(res => {
            if (res.type == "delete") {
                var index = this.productassumption.findIndex(x => x.id === item.id);
