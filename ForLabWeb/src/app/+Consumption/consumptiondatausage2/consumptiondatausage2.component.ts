...
@Component({
  selector: 'app-consumptiondatausage2',
  templateUrl: './consumptiondatausage2.component.html',
  styles: ['.table tr.active td {   background-color:#123456 !important;    color: white;  }'],
})
export class Consumptiondatausage2Component implements OnInit {
...
this.param="";
    ...
    this._APIwithActionService.getDatabyID(this.forecastid,"Consumption","Gettotalsiteandproduct").subscribe((data)=>{
      ...
      this.totalproductno=data.totalproductno
    ...
    this._APIwithActionService.deleteData(this.forecastid, "Consumption", "removestefromdatausage", "siteid=" + item.siteID).subscribe((data) => {
      ...
      if (type == 's') {
        ...
        if (siteindex >= 0) {
          ...
          this._GlobalAPIService.SuccessMessage(item.siteName + " Reportedsite deleted successfully");
        }
      }
      else {
        ...
        if (siteindex >= 0) {
          ...
          this._GlobalAPIService.SuccessMessage(item.siteName + " NonReportedsite deleted successfully");
        }
      }
    })
